---
title: "Microglia extraction from Milich et al."
author: "James Choi"
...
results_out <- '../results/MicrogliaExtraction_Milich2021/'
...
```{r}
Milich2021 <- readRDS(file = paste0(data_out, 'Milich2021.rds'))
```

```{r}
dat <- CreateSeuratObject(
  counts = Milich2021$counts,
  project = 'Milich2021',
  meta.data = Milich2021$metadata
)
dat <- dat %>% 
  NormalizeData() %>% 
...
p4 <- (p1 | p2 | p3)
# p4
ggsave(filename = paste0(results_out, 'Milich2021_metadataUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Milich2021_MicrogliaMarkerUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Milich2021_DAMMarkerUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Milich2021_NonMicrogliaMarkerUMAP.tiff'),
...
```{r}
if (all(rownames(dat@meta.data) == rownames(Milich2021$metadata))) {
  Milich2021$metadata$celltype <- plyr::mapvalues(
    x = dat@meta.data$RNA_snn_res.0.6,
    from = levels(dat@meta.data$RNA_snn_res.0.6),
    to = rep('Microglia', length(levels(dat@meta.data$RNA_snn_res.0.6)))
  )
...
```{r}
nMG <- table(Milich2021$metadata$celltype)['Microglia']
...
p2 <- DimPlot(dat, group.by = 'RNA_snn_res.0.6', shuffle = TRUE,
  ...
  my_theme
p3 <- (p1 | p2) 
ggsave(filename = paste0(results_out, 'Milich2021_celltypeUMAP.tiff'),
