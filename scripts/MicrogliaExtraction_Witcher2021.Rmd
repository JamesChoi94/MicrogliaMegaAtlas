---
title: "Microglia extraction from Witcher et al."
author: "James Choi"
...
results_out <- '../results/MicrogliaExtraction_Witcher2021/'
...
```{r}
Witcher2021 <- readRDS(file = paste0(data_out, 'Witcher2021.rds'))
```

```{r}
dat <- CreateSeuratObject(
  counts = Witcher2021$counts,
  project = 'Witcher2021',
  meta.data = Witcher2021$metadata
)
dat <- dat %>% 
  NormalizeData() %>% 
...
p4 <- (p1 | p2 | p3)
# p4
ggsave(filename = paste0(results_out, 'Witcher2021_metadataUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Witcher2021_MicrogliaMarkerUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Witcher2021_DAMMarkerUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Witcher2021_NonMicrogliaMarkerUMAP.tiff'),
       plot = p3, device = 'tiff', height = 7.5, width = 12.5)
```


```{r}
tmp_deg <- FindMarkers(dat, c(1,5), only.pos = TRUE, logfc.threshold = 2)
# cluster 1, 5 are 

tmp_deg <- FindMarkers(dat, c(6), only.pos = TRUE, logfc.threshold = 1)
...
```{r}
if (all(rownames(dat@meta.data) == rownames(Witcher2021$metadata))) {
  Witcher2021$metadata$celltype <- plyr::mapvalues(
...
```{r}
nMG <- table(Witcher2021$metadata$celltype)['Microglia']
...
p2 <- DimPlot(dat, group.by = 'RNA_snn_res.0.6', shuffle = TRUE,
  ...
  my_theme
p3 <- (p1 | p2) 
ggsave(filename = paste0(results_out, 'Witcher2021_celltypeUMAP.tiff'),
