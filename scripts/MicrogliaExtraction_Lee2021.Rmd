---
title: "Microglia extraction from Lee et al."
author: "James Choi"
...
results_out <- '../results/MicrogliaExtraction_Lee2021/'
...
```{r}
Lee2021 <- readRDS(file = paste0(data_out, 'Lee2021.rds'))
```

```{r}
genes <- rownames(Lee2021$counts)
library('biomaRt')
...
rownames(Lee2021$counts) <- plyr::mapvalues(
  x = rownames(Lee2021$counts),
  from = tmp_convert$ensembl_gene_id,
  to = tmp_convert$mgi_symbol,
  warn_missing = FALSE
)
Lee2021$counts <- Lee2021$counts[nchar(rownames(Lee2021$counts)) > 0,]
```
...
dat <- CreateSeuratObject(
  counts = Lee2021$counts,
  project = 'Lee2021',
  meta.data = Lee2021$metadata
)
dat <- dat %>% 
  NormalizeData() %>% 
...
p4 <- (p1 | p2 | p3)
# p4
ggsave(filename = paste0(results_out, 'Lee2021_metadataUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Lee2021_MicrogliaMarkerUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Lee2021_DAMMarkerUMAP.tiff'),
...
ggsave(filename = paste0(results_out, 'Lee2021_NonMicrogliaMarkerUMAP.tiff'),
...
```{r}
tmp_deg <- FindMarkers(dat, ident.1 = 0, only.pos = TRUE, 
                       logfc.threshold = 1)
...
```{r}
if (all(rownames(dat@meta.data) == rownames(Lee2021$metadata))) {
  Lee2021$metadata$celltype <- plyr::mapvalues(
...
```{r}
nMG <- table(Lee2021$metadata$celltype)['Microglia']
...
p2 <- DimPlot(dat, group.by = 'RNA_snn_res.0.6', shuffle = TRUE,
  ...
  my_theme
p3 <- (p1 | p2)
ggsave(filename = paste0(results_out, 'Lee2021_celltypeUMAP.tiff'),
